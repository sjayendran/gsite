...
Meteor.methods({
  ...
  getfriendList: function() {
	...
	},//To cross check validity of FB link list with YouTube, to only bring in MUSIC links
						...
						if(groovitFBResultSet[gCount].attribution == "Groovit") {
							...
							try{
								Videos.insert({
									...
									st : trackQuery,
									count: 1, 
									sharedBy : [groovitFBResultSet[gCount].post_id.split("_")[0].toString()+"||"+millisecondDate]
								});
							}
							catch(err)
							{
								...
								else if(err.message.indexOf("E11000 duplicate key error") >= 0)
								{
									...
									Videos.update({_id:groovitFBResultSet[gCount].attachment.media[0].href}, {$set: {dataSource: "FB"}, $push: {sharedBy: groovitFBResultSet[gCount].post_id.split("_")[0].toString()+"||"+millisecondDate}});
								}
								//return "error";
							}
									...
									try{
										if(LFMlargeAlbumArt != "" && LFMmediumAlbumArt == "")
										{
											...
											{sa: LFMArtist, 
											 st: LFMTitle,
											 dataSource : "FB",
											 largeAlbumArt: LFMlargeAlbumArt}
											);
										}
										else if(LFMlargeAlbumArt != "" && LFMmediumAlbumArt != "")
										{
											...
											{sa: LFMArtist, 
											 ...
											 dataSource : "FB",
											 largeAlbumArt: LFMlargeAlbumArt,
											 mediumAlbumArt: LFMmediumAlbumArt}
											);
										}
